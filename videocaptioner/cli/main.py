...
def _build_transcribe_parser(subparsers) -> None:
    ...
    p.add_argument("--qwen-no-vad", action="store_true", help=argparse.SUPPRESS)
...
def _build_cli_overrides(args: argparse.Namespace) -> dict:
    ...
    if getattr(args, "qwen_no_low_memory", False):
        _set("transcribe.qwen3_asr.low_memory", False)
    if getattr(args, "qwen_no_vad", False):
        _set("transcribe.qwen3_asr.vad_filter", False)
