...
def _create_asr_instance(audio_path: str, config: TranscribeConfig) -> Union[ChunkedASR, BaseASR]:
    ...
    elif model_type == TranscribeModelEnum.QWEN3_ASR:
        return _create_qwen3_asr(audio_path, config)
...
def _create_qwen3_asr(audio_path: str, config: TranscribeConfig) -> Qwen3ASR:
    """Create the isolated local Qwen3-ASR implementation."""
    return Qwen3ASR(
        audio_input=audio_path,
        use_cache=True,
        need_word_time_stamp=config.need_word_time_stamp,
        runtime_python=config.qwen_asr_runtime_python,
        asr_model_dir=config.qwen_asr_model_dir,
        aligner_model_dir=config.qwen_asr_aligner_model_dir,
        language=config.transcribe_language,
