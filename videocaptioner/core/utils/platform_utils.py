...
def get_available_transcribe_models() -> list[TranscribeModelEnum]:
    ...
    unavailable = set()
    if is_macos():
        unavailable.add(TranscribeModelEnum.FASTER_WHISPER)
    if not is_windows():
        unavailable.add(TranscribeModelEnum.QWEN3_ASR)
    return [model for model in all_models if model not in unavailable]
...
def is_model_available(model: TranscribeModelEnum) -> bool:
    ...
    if not is_windows() and model == TranscribeModelEnum.QWEN3_ASR:
        return False
